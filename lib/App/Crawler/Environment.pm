package App::Crawler::Environment;
...
BEGIN {
  if (not ($ENV{DANCER_APPDIR} || '')
      ...
      my $uid = (stat($me))[4] || 0;
      my $home = ($ENV{CRAWLER_HOME} || (getpwuid($uid))[7] || $ENV{HOME});
      $ENV{CRAWLER_HOME} ||= $home;

      $ENV{CRAWLER_DO} ||= File::Spec->catfile($FindBin::RealBin, 'crawler-do');

      my $auto = dir(dist_dir('App-Crawler'))->absolute;
