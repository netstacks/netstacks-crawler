package App::Crawler::Worker::Loader;
...
sub load_workers {
  ...
  foreach my $plugin (@user_plugins, @core_plugins) {
    $plugin =~ s/^X::/+App::CrawlerX::Worker::Plugin::/;
    $plugin = 'App::Crawler::Worker::Plugin::'. $plugin
      if $plugin !~ m/^\+/;
    $plugin =~ s/^\+//;
  ...
  if (setting('enable_python_worklets')) {
      ...
      Module::Load::load 'App::Crawler::Worker::Plugin::PythonShim', $action;
  }
