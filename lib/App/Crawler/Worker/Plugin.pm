package App::Crawler::Worker::Plugin;
...
register 'register_worker' => sub {
  ...
  my $package = (caller)[0];
  ($workerconf->{package} = $package) =~ s/^App::Crawler::Worker::Plugin:://;
...
App::Crawler::Worker::Plugin - Netdisco Workers

=head1 Introduction

L<App::Crawler>'s plugin system allows users to write I<workers> to gather
