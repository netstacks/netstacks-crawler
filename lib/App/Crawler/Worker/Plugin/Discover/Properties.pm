package App::Crawler::Worker::Plugin::Discover::Properties;
...
use App::Crawler::Util::FastResolver 'hostnames_resolve_async';
use App::Crawler::Util::Device qw/get_device is_discoverable/;
use App::Crawler::Util::DNS 'hostname_from_ip';
use App::Crawler::Util::SNMP qw/snmp_comm_reindex get_mibdirs_shortnames/;
use App::Crawler::Util::Web 'sort_port';
use App::Crawler::DB::ExplicitLocking ':modes';
...
register_worker({ phase => 'early', driver => 'snmp',
  ...
  if ((not $device->model or $device->model eq 'unknown'
      ...
      if ($object) {
          ...
          my $file = file((setting('mibhome') || dir(($ENV{CRAWLER_HOME} || $ENV{HOME}), 'netdisco-mibs')),
...
register_worker({ phase => 'early', driver => 'snmp',
  ...
  # EXCLUSIVE, not ACCESS EXCLUSIVE: the delete-then-repopulate runs in one
  # transaction, so MVCC already hides the intermediate empty-ports window from
  # readers (they see the pre-commit snapshot until COMMIT). ACCESS EXCLUSIVE is
  # the only mode that also blocks plain SELECTs — it made the whole SPA/API
  # freeze for the duration of a large device's discover. EXCLUSIVE still
  # serializes concurrent writers (other discovers, port control) but lets the
  # UI's ACCESS SHARE reads through.
  #
  # The LOCK and the delete/populate below MUST run on the same DB connection.
  # $device is loaded via schema(vars->{'tenant'}) (see get_device), which is a
  # distinct DBIC connection from schema('netdisco'). Taking the table lock on
  # schema('netdisco') while $device->ports->delete runs on the tenant
  # connection self-deadlocks: the netdisco connection holds the EXCLUSIVE lock
  # and sits idle while the tenant connection blocks forever on its own
  # sibling's lock (Postgres can't see it as a deadlock). Lock on $device's own
  # schema so both statements share one connection.
  $device->result_source->schema->resultset('DevicePort')->txn_do_locked(EXCLUSIVE, sub {
    my $coder = JSON::PP->new->utf8(0)->allow_nonref(1)->allow_unknown(1);
