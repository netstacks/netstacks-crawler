package App::Crawler::Worker::Plugin::Discover::CanonicalIP;
...
use App::Crawler::Util::Device 'is_discoverable';
...
register_worker({ phase => 'main', driver => 'snmp' }, sub {
  ...
  if (setting('reverse_sysname') and $revofname) {
    if (App::Crawler::Transport::SNMP->test_connection( $new_ip )) {
      $new_ip = $revofname;
    }
    else {
    ...
    ALIASMAP: foreach my $map (@idmaps) {
      ...
      foreach my $key (sort keys %$map) {
        ...
        foreach my $alias (@devips) {
          ...
          if (App::Crawler::Transport::SNMP->test_connection( $alias->alias )) {
            $new_ip = $alias->alias;
            last ALIASMAP;
          }
          else {
