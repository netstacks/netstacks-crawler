package App::Crawler::Worker::Plugin::MakeRancidConf;
...
register_worker({ phase => 'main' }, sub {
  ...
  my $rancidcvsroot = $config->{rancid_cvsroot}
    || dir($ENV{CRAWLER_HOME}, 'rancid')->stringify;
  ...
  foreach my $group (keys %$routerdb) {
    ...
    $content .= "# Generate automatically by App::Crawler::Worker::Plugin::MakeRancidConf\n#\n";
    $content .= join "\n", sort @{$routerdb->{$group}};
...
Since MakeRancidConf is a worker module it can also be run via C<crawler-do>:

 ~/bin/crawler-do makerancidconf

Skipped devices and the reason for skipping them can be seen by using C<-D>:

 ~/bin/crawler-do makerancidconf -D
...
C<rancid>. All keys are optional:
 ...
 rancid:
   rancid_cvsroot:  '$ENV{CRAWLER_HOME}/rancid' # default
...
C<crawler-rancid-export> script).
