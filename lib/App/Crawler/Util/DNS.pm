...
sub hostname_from_ip {
  my ($ip, $opts) = @_;
  return unless $ip;
  return if setting('dns')->{'disabled'};
...
sub ipv4_from_hostname {
  ...
  return undef if setting('dns')->{'disabled'};
  my $ETCHOSTS = setting('dns')->{'ETCHOSTS'};
...
# to avoid circular dependency with App::Crawler::Util::Permission
