package App::Crawler::Configuration;

use App::Crawler::Environment;
use App::Crawler::Util::DeviceAuth ();
use Dancer ':script';
use Exporter 'import';
our @EXPORT_OK = qw(merge_env_overrides);
...
BEGIN {
  if (setting('include_paths') and ref [] eq ref setting('include_paths')) {
    ...
    push @{setting('include_paths')},
         dir(($ENV{CRAWLER_HOME} || $ENV{HOME}), 'nd-site-local', 'lib')->stringify
...
if (ref {} eq ref setting('database')) {
    ...
    setting('database')->{name} =
      ($ENV{CRAWLER_DB_NAME} || $ENV{CRAWLER_DBNAME} || $ENV{PGDATABASE} || setting('database')->{name});

    setting('database')->{host} =
      ($ENV{CRAWLER_DB_HOST} || $ENV{PGHOST} || setting('database')->{host});

    my $portnum = ($ENV{CRAWLER_DB_PORT} || $ENV{PGPORT});
    ...
    setting('database')->{user} =
      ($ENV{CRAWLER_DB_USER} || $ENV{PGUSER} || setting('database')->{user});

    setting('database')->{pass} =
      ($ENV{CRAWLER_DB_PASS} || $ENV{PGPASSWORD} || setting('database')->{pass});
    ...
    setting('plugins')->{DBIC}->{'default'}
     = setting('plugins')->{DBIC}->{$ENV{CRAWLER_DB_TENANT}}
     if $ENV{CRAWLER_DB_TENANT}
        and $ENV{CRAWLER_DB_TENANT} ne 'netdisco'
        and exists setting('plugins')->{DBIC}->{$ENV{CRAWLER_DB_TENANT}};
    ...
    # must happen after tenants as this rewrites env if CRAWLER_DB_TENANT in play
    my $default = setting('plugins')->{DBIC}->{'default'};
    if ($default->{dsn} =~ m/dbname=([^;]+)/) {
        $ENV{PGDATABASE} = $1;
        $ENV{CRAWLER_DB_NAME} = $ENV{PGDATABASE};
    }
    if ($default->{dsn} =~ m/host=([^;]+)/) {
        $ENV{PGHOST} = $1;
        $ENV{CRAWLER_DB_HOST} = $ENV{PGHOST};
    }
    if ($default->{dsn} =~ m/port=(\d+)/) {
        $ENV{PGPORT} = $1;
        $ENV{CRAWLER_DB_PORT} = $ENV{PGPORT};
    }
    $ENV{PGUSER} = $default->{user};
    $ENV{CRAWLER_DB_USER} = $ENV{PGUSER};
    $ENV{PGPASSWORD} = $default->{password};
    $ENV{CRAWLER_DB_PASS} = $ENV{PGPASSWORD};
...
# override from env for docker
config->{'community'} = ($ENV{CRAWLER_RO_COMMUNITY} ?
  [split ',', $ENV{CRAWLER_RO_COMMUNITY}] : config->{'community'});
config->{'community_rw'} = ($ENV{CRAWLER_RW_COMMUNITY} ?
  [split ',', $ENV{CRAWLER_RW_COMMUNITY}] : config->{'community_rw'});
...
config->{'device_auth'}
  = [ App::Crawler::Util::DeviceAuth::fixup_device_auth() ];
...
# override from env for docker
if ($ENV{CRAWLER_DOMAIN}) {
  if ($ENV{CRAWLER_DOMAIN} eq 'discover') {
    delete $ENV{CRAWLER_DOMAIN};
    ...
    config->{'domain_suffix'} = $ENV{CRAWLER_DOMAIN};
  }
}

# override SNMP bulkwalk from environment
config->{'bulkwalk_off'} = true
  if (exists $ENV{CRAWLER_SNMP_BULKWALK_OFF} and $ENV{CRAWLER_SNMP_BULKWALK_OFF});
...
#config->{plugins}->{Swagger}->{ui_dir} =
  #dir(dist_dir('App-Crawler'), 'share', 'public', 'swagger-ui')->absolute;
...
sub merge_env_overrides {
    my ($cfg, $env) = @_;

    # 1) DATABASE_URL → cfg->database
    if (my $url = $env->{DATABASE_URL}) {
        if ($url =~ m{^postgres(?:ql)?://(?:([^:]+)(?::([^@]*))?@)?([^:/]+)(?::(\d+))?/([^?]+)}) {
            my ($u, $p, $h, $port, $db) = ($1, $2, $3, $4, $5);
            $cfg->{database} ||= {};
            $cfg->{database}{dsn}  = "dbi:Pg:dbname=$db;host=$h" . ($port ? ";port=$port" : '');
            $cfg->{database}{user} = $u if defined $u;
            $cfg->{database}{pass} = $p if defined $p;
            $cfg->{database}{host} = $h;
            # Netdisco's internal config logic rebuilds dsn from database->{name},
            # so set it explicitly to avoid clobbering our dsn with an empty dbname.
            $cfg->{database}{name} = $db;
            # Legacy top-level keys some Netdisco internals read directly
            $cfg->{db_user} = $u if defined $u;
            $cfg->{db_pass} = $p if defined $p;
            $cfg->{db_name} = $db;
            $cfg->{db_host} = $h;
        }
    }

    # 2) CRAWLER_<KEY1>__<KEY2>=value → cfg->{key1}{key2} = value
    for my $name (sort keys %$env) {
        next unless $name =~ /^CRAWLER_(.+)$/;
